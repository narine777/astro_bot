"""
🚀 AstroBot: Полный справочник по астрономии с решениями задач
🎯 Солнечная система + звезды для олимпиад
"""

import os
import logging
from telegram import Update, ReplyKeyboardMarkup, KeyboardButton, InlineKeyboardMarkup, InlineKeyboardButton
from telegram.ext import Application, CommandHandler, ContextTypes, CallbackQueryHandler, MessageHandler, filters

# Настройка логирования
logging.basicConfig(
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    level=logging.INFO
)
logger = logging.getLogger(__name__)

TOKEN = os.environ.get('BOT_TOKEN')
if not TOKEN:
    print("❌ ОШИБКА: Переменная окружения BOT_TOKEN не установлена!")
    print("Добавьте её в Railway → Variables")
    exit(1)

# БАЗА ДАННЫХ НЕБЕСНЫХ ТЕЛ
CELESTIAL_BODIES = {
    "Сириус": {
        "emoji": "⭐️",
        "name_en": "Sirius",
        "type": "Двойная звезда (Сириус A + Сириус B)",
        "mass": "2.06 M☉ (Сириус A) + 0.98 M☉ (Сириус B)",
        "radius": "1.71 R☉ (Сириус A), 0.0084 R☉ (Сириус B)",
        "luminosity": "25.4 L☉ (Сириус A), 0.056 L☉ (Сириус B)",
        "distance": "2.64 парсека (8.60 световых лет)",
        "sources": "SIMBAD, NASA ADS, Gaia DR3",
        "accuracy": "Высокая (±1%)",
        "task": "Задача: Рассчитайте общую массу системы Сириус.",
        "solution": """
📝 **Решение задачи:**
Mобщ = M_A + M_B
Mобщ = 2.06 M☉ + 0.98 M☉ = 3.04 M☉

📐 **Используемая формула:**
Mобщ = ΣMᵢ (сумма масс компонентов)

🎯 **Ответ:** ~3.04 M☉
""",
        "formula": "Mобщ = M_A + M_B"
    },
    "Солнце": {
        "emoji": "☀️",
        "name_en": "Sun",
        "type": "Жёлтый карлик (G2V)",
        "mass": "1.9885×10³⁰ кг (1 M☉)",
        "radius": "695 700 км (1 R☉)",
        "luminosity": "3.828×10²⁶ Вт (1 L☉)",
        "temperature": "5772 K (эффективная)",
        "sources": "NASA, SOHO, CODATA",
        "accuracy": "Очень высокая (±0.001%)",
        "task": "Задача: Сколько энергии получает Земля от Солнца?",
        "solution": """
📝 **Решение задачи:**
1. Солнечная постоянная: S = 1361 Вт/м²
2. Площадь сечения Земли: A = πR²
   R⊕ = 6.371×10⁶ м
   A = π × (6.371×10⁶)² ≈ 1.275×10¹⁴ м²
3. Энергия: P = S × A
   P = 1361 × 1.275×10¹⁴ ≈ 1.74×10¹⁷ Вт

📐 **Используемые формулы:**
- Площадь круга: A = πR²
- Мощность: P = S × A

🎯 **Ответ:** ~1.74×10¹⁷ Вт
""",
        "formula": "P = S × πR², где S = 1361 Вт/м²"
    },
    "Меркурий": {
        "emoji": "☿",
        "name_en": "Mercury",
        "type": "Планета земной группы",
        "mass": "3.3011×10²³ кг (0.0553 M⊕)",
        "radius": "2439.7 км",
        "distance": "0.3871 а.е. от Солнца",
        "period": "88 земных суток",
        "sources": "NASA Fact Sheet, MESSENGER mission",
        "accuracy": "Высокая (±0.01%)",
        "task": "Задача: Ускорение свободного падения на Меркурии?",
        "solution": """
📝 **Решение задачи:**
1. Формула: g = GM/R²
2. Константы:
   G = 6.67430×10⁻¹¹ м³/(кг·с²)
   M = 3.3011×10²³ кг
   R = 2.4397×10⁶ м
3. Расчет:
   g = (6.67430×10⁻¹¹ × 3.3011×10²³) / (2.4397×10⁶)²
   g ≈ 3.70 м/с²

📐 **Используемая формула:**
g = GM/R²

🎯 **Ответ:** ~3.70 м/с²
""",
        "formula": "g = GM/R²"
    },
    "Венера": {
        "emoji": "♀",
        "name_en": "Venus",
        "type": "Планета земной группы",
        "mass": "4.8675×10²⁴ кг (0.815 M⊕)",
        "radius": "6051.8 км",
        "distance": "0.7233 а.е. от Солнца",
        "period": "224.7 земных суток",
        "sources": "NASA, Venus Express",
        "accuracy": "Очень высокая (±0.001%)",
        "task": "Задача: Вторая космическая скорость для Венеры?",
        "solution": """
📝 **Решение задачи:**
1. Формула: v₂ = √(2GM/R)
2. Константы:
   G = 6.67430×10⁻¹¹ м³/(кг·с²)
   M = 4.8675×10²⁴ кг
   R = 6.0518×10⁶ м
3. Расчет:
   v₂ = √(2 × 6.67430×10⁻¹¹ × 4.8675×10²⁴ / 6.0518×10⁶)
   v₂ ≈ √(6.494×10⁷) ≈ 1.036×10⁴ м/с

📐 **Используемая формула:**
v₂ = √(2GM/R)

🎯 **Ответ:** ~10.36 км/с
""",
        "formula": "v₂ = √(2GM/R)"
    },
    "Земля": {
        "emoji": "🌍",
        "name_en": "Earth",
        "type": "Планета земной группы",
        "mass": "5.9722×10²⁴ кг (1 M⊕)",
        "radius": "6371.0 км (средний)",
        "distance": "1.000001018 а.е. от Солнца",
        "period": "365.256 суток",
        "sources": "NASA/GSFC, CODATA 2018",
        "accuracy": "Очень высокая (±0.0001%)",
        "task": "Задача: Линейная скорость точки на экваторе?",
        "solution": """
📝 **Решение задачи:**
1. Формула: v = 2πR/T
2. Константы:
   R = 6.371×10⁶ м
   T = 24 часа = 86400 секунд
3. Расчет:
   v = (2 × π × 6.371×10⁶) / 86400
   v ≈ (4.000×10⁷) / 86400 ≈ 463 м/с

📐 **Используемая формула:**
v = 2πR/T (равномерное движение по окружности)

🎯 **Ответ:** ~463 м/с
""",
        "formula": "v = 2πR/T"
    },
    "Марс": {
        "emoji": "♂",
        "name_en": "Mars",
        "type": "Планета земной группы",
        "mass": "6.4171×10²³ кг (0.107 M⊕)",
        "radius": "3389.5 км",
        "distance": "1.5237 а.е. от Солнца",
        "period": "687 земных суток",
        "sources": "NASA, Mars Reconnaissance Orbiter",
        "accuracy": "Высокая (±0.01%)",
        "task": "Задача: Сила тяжести на Марсе?",
        "solution": """
📝 **Решение задачи:**
1. Формула: g = GM/R²
2. Константы:
   G = 6.67430×10⁻¹¹ м³/(кг·с²)
   M = 6.4171×10²³ кг
   R = 3.3895×10⁶ м
3. Расчет:
   g = (6.67430×10⁻¹¹ × 6.4171×10²³) / (3.3895×10⁶)²
   g ≈ 3.71 м/с²

📐 **Используемая формула:**
g = GM/R²

🎯 **Ответ:** ~3.71 м/с² (38% земной)
""",
        "formula": "g = GM/R²"
    },
    "Юпитер": {
        "emoji": "♃",
        "name_en": "Jupiter",
        "type": "Газовый гигант",
        "mass": "1.8982×10²⁷ кг (317.8 M⊕)",
        "radius": "69 911 км (экваториальный)",
        "distance": "5.2034 а.е. от Солнца",
        "period": "11.86 земных лет",
        "sources": "NASA, Juno mission",
        "accuracy": "Высокая (±0.01%)",
        "task": "Задача: Во сколько раз Юпитер массивнее Земли?",
        "solution": """
📝 **Решение задачи:**
1. Данные:
   M_Юпитер = 1.8982×10²⁷ кг
   M_Земля = 5.9722×10²⁴ кг
2. Расчет отношения:
   N = M_Юпитер / M_Земля
   N = 1.8982×10²⁷ / 5.9722×10²⁴
   N ≈ 317.8

📐 **Используемая формула:**
N = M₁/M₂

🎯 **Ответ:** ~318 раз
""",
        "formula": "N = M₁/M₂"
    },
    "Сатурн": {
        "emoji": "♄",
        "name_en": "Saturn",
        "type": "Газовый гигант",
        "mass": "5.6834×10²⁶ кг (95.16 M⊕)",
        "radius": "58 232 км (экваториальный)",
        "distance": "9.5826 а.е. от Солнца",
        "period": "29.46 земных лет",
        "sources": "NASA, Cassini mission",
        "accuracy": "Высокая (±0.02%)",
        "task": "Задача: Средняя плотность Сатурна?",
        "solution": """
📝 **Решение задачи:**
1. Формула: ρ = M/V = 3M/(4πR³)
2. Константы:
   M = 5.6834×10²⁶ кг
   R = 5.8232×10⁷ м
3. Расчет:
   V = (4/3)πR³ = (4/3)π × (5.8232×10⁷)³ ≈ 8.271×10²³ м³
   ρ = 5.6834×10²⁶ / 8.271×10²³ ≈ 687 кг/м³

📐 **Используемая формула:**
ρ = 3M/(4πR³)

🎯 **Ответ:** ~0.687 г/см³
""",
        "formula": "ρ = 3M/(4πR³)"
    },
    "Уран": {
        "emoji": "♅",
        "name_en": "Uranus",
        "type": "Ледяной гигант",
        "mass": "8.6810×10²⁵ кг (14.54 M⊕)",
        "radius": "25 362 км",
        "distance": "19.201 а.е. от Солнца",
        "period": "84.01 земных лет",
        "sources": "NASA, Voyager 2",
        "accuracy": "Средняя (±0.1%)",
        "task": "Задача: Сравните массу Урана и Нептуна",
        "solution": """
📝 **Решение задачи:**
1. Данные:
   M_Уран = 8.6810×10²⁵ кг
   M_Нептун = 1.0241×10²⁶ кг
2. Расчет отношения:
   N = M_Уран / M_Нептун
   N = 8.6810×10²⁵ / 1.0241×10²⁶ ≈ 0.848
3. Вывод:
   Уран легче Нептуна примерно на 15%

📐 **Используемая формула:**
N = M₁/M₂

🎯 **Ответ:** Уран легче (~14.5 M⊕ vs ~17.1 M⊕)
""",
        "formula": "N = M₁/M₂"
    },
    "Нептун": {
        "emoji": "♆",
        "name_en": "Neptune",
        "type": "Ледяной гигант",
        "mass": "1.0241×10²⁶ кг (17.15 M⊕)",
        "radius": "24 622 км",
        "distance": "30.07 а.е. от Солнца",
        "period": "164.8 земных лет",
        "sources": "NASA, Voyager 2",
        "accuracy": "Средняя (±0.1%)",
        "task": "Задача: Какая планета дальше - Уран или Нептун?",
        "solution": """
📝 **Решение задачи:**
1. Данные:
   a_Уран = 19.201 а.е.
   a_Нептун = 30.07 а.е.
2. Сравнение:
   30.07 а.е. > 19.201 а.е.
   Разница: 30.07 - 19.201 = 10.869 а.е.
3. Вывод:
   Нептун дальше от Солнца, чем Уран

📐 **Используемые данные:**
Большая полуось орбиты (a)

🎯 **Ответ:** Нептун (30.07 а.е. > 19.20 а.е.)
""",
        "formula": "Сравнение: a_Нептун > a_Уран"
    }
}


# ==================== КЛАВИАТУРЫ ====================

def get_main_keyboard():
    """Основная клавиатура"""
    keyboard = [
        [KeyboardButton("🪐 8 Планет"), KeyboardButton("⭐️ Сириус"), KeyboardButton("☀️ Солнце")],
        [KeyboardButton("📊 Сравнить"), KeyboardButton("📝 Задачи"), KeyboardButton("🔬 Методы")],
        [KeyboardButton("❓ Помощь")]
    ]
    return ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=False)


def get_planets_keyboard():
    """Клавиатура с 8 планетами (без Сириуса и Солнца)"""
    keyboard = [
        [InlineKeyboardButton("☿ Меркурий", callback_data="body_Меркурий"),
         InlineKeyboardButton("♀ Венера", callback_data="body_Венера")],
        [InlineKeyboardButton("🌍 Земля", callback_data="body_Земля"),
         InlineKeyboardButton("♂ Марс", callback_data="body_Марс")],
        [InlineKeyboardButton("♃ Юпитер", callback_data="body_Юпитер"),
         InlineKeyboardButton("♄ Сатурн", callback_data="body_Сатурн")],
        [InlineKeyboardButton("♅ Уран", callback_data="body_Уран"),
         InlineKeyboardButton("♆ Нептун", callback_data="body_Нептун")],
        [InlineKeyboardButton("🔙 Назад в меню", callback_data="back_main")]
    ]
    return InlineKeyboardMarkup(keyboard)


def get_compare_keyboard():
    """Клавиатура для сравнения"""
    keyboard = [
        [InlineKeyboardButton("Земля vs Марс", callback_data="compare_Земля_Марс"),
         InlineKeyboardButton("Венера vs Земля", callback_data="compare_Венера_Земля")],
        [InlineKeyboardButton("Юпитер vs Сатурн", callback_data="compare_Юпитер_Сатурн"),
         InlineKeyboardButton("Солнце vs Сириус", callback_data="compare_Солнце_Сириус")],
        [InlineKeyboardButton("🔙 Назад в меню", callback_data="back_main")]
    ]
    return InlineKeyboardMarkup(keyboard)


def get_tasks_keyboard():
    """Клавиатура с задачами"""
    keyboard = [
        [InlineKeyboardButton("📐 Космические скорости", callback_data="task_velocity"),
         InlineKeyboardButton("⚖️ Сравнение масс", callback_data="task_mass")],
        [InlineKeyboardButton("🌍 Сила тяжести", callback_data="task_gravity"),
         InlineKeyboardButton("🔄 Периоды", callback_data="task_period")],
        [InlineKeyboardButton("⭐️ Звездные задачи", callback_data="task_stars"),
         InlineKeyboardButton("📏 Плотность", callback_data="task_density")],
        [InlineKeyboardButton("🔙 Назад в меню", callback_data="back_main")]
    ]
    return InlineKeyboardMarkup(keyboard)


# ==================== ОБРАБОТЧИКИ СООБЩЕНИЙ ====================

async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /start"""
    welcome = f"""
🚀 *Добро пожаловать в AstroBot!*
*Справочник для олимпиадной астрономии*

*Доступные объекты:*
• ☀️ **Солнце** - наша звезда
• 🪐 **8 Планет** - от Меркурия до Нептуна
• ⭐️ **Сириус** - самая яркая звезда

*Функции:*
📊 **Сравнить** - сравнение двух объектов
📝 **Задачи** - олимпиадные задачи с решениями
🔬 **Методы** - методики измерений
❓ **Помощь** - справка по боту

*Нажмите кнопку ниже для начала:*
"""
    await update.message.reply_text(welcome, parse_mode='Markdown', reply_markup=get_main_keyboard())


async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработчик текстовых сообщений"""
    text = update.message.text

    if text == "🪐 8 Планет":
        await update.message.reply_text(
            "🌌 *Выберите планету:*\n(8 планет Солнечной системы)",
            parse_mode='Markdown',
            reply_markup=get_planets_keyboard()
        )

    elif text == "⭐️ Сириус":
        await show_celestial_body_direct(update, "Сириус")

    elif text == "☀️ Солнце":
        await show_celestial_body_direct(update, "Солнце")

    elif text == "📊 Сравнить":
        await update.message.reply_text(
            "⚖️ *Выберите пару для сравнения:*",
            parse_mode='Markdown',
            reply_markup=get_compare_keyboard()
        )

    elif text == "📝 Задачи":
        await update.message.reply_text(
            "📚 *Выберите тип задачи:*\n\n"
            "📐 *Космические скорости* - первая и вторая космические\n"
            "⚖️ *Сравнение масс* - отношения масс объектов\n"
            "🌍 *Сила тяжести* - ускорение свободного падения\n"
            "🔄 *Периоды* - орбитальные и синодические периоды\n"
            "⭐️ *Звездные задачи* - светимость, масса звезд\n"
            "📏 *Плотность* - средняя плотность планет",
            parse_mode='Markdown',
            reply_markup=get_tasks_keyboard()
        )

    elif text == "🔬 Методы":
        await show_methods(update)

    elif text == "❓ Помощь":
        await show_help(update)

    else:
        await update.message.reply_text(
            "Пожалуйста, используйте кнопки меню ⬇️",
            reply_markup=get_main_keyboard()
        )


async def show_celestial_body_direct(update: Update, body_name: str):
    """Непосредственно показать информацию о небесном теле"""
    if body_name not in CELESTIAL_BODIES:
        await update.message.reply_text(
            f"❌ Объект '{body_name}' не найден.",
            reply_markup=get_main_keyboard()
        )
        return

    body = CELESTIAL_BODIES[body_name]

    response = f"{body['emoji']} *{body_name.upper()}* ({body['name_en']})\n\n"
    response += f"📌 *Тип:* {body['type']}\n\n"

    response += f"⚖️ *Масса:* {body['mass']}\n"
    response += f"📏 *Радиус:* {body['radius']}\n"

    if 'distance' in body:
        response += f"📍 *Расстояние:* {body['distance']}\n"

    if 'period' in body:
        response += f"🔄 *Период обращения:* {body['period']}\n"

    if 'luminosity' in body:
        response += f"☀️ *Светимость:* {body['luminosity']}\n"

    if 'temperature' in body:
        response += f"🌡️ *Температура:* {body['temperature']}\n"

    response += f"\n📊 *Точность:* {body['accuracy']}\n"
    response += f"📚 *Источники:* {body['sources']}\n\n"
    response += f"🎯 *{body['task']}*\n\n"
    response += body['solution']
    response += "\n_Используйте данные для решения олимпиадных задач!_"

    await update.message.reply_text(response, parse_mode='Markdown', reply_markup=get_main_keyboard())


async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработчик нажатий на инлайн-кнопки"""
    query = update.callback_query
    await query.answer()

    data = query.data

    if data.startswith("body_"):
        body_name = data.split("_")[1]
        await show_celestial_body_inline(query, body_name)

    elif data.startswith("compare_"):
        bodies = data.split("_")[1:]
        if len(bodies) == 2:
            await show_comparison(query, bodies[0], bodies[1])

    elif data.startswith("task_"):
        task_type = data.split("_")[1]
        await show_task_with_solution(query, task_type)

    elif data == "back_main":
        await query.edit_message_text(
            "🏠 *Возврат в главное меню*\nВыберите действие из кнопок ниже:",
            parse_mode='Markdown'
        )
        await query.edit_message_reply_markup(None)

        # Отправляем основную клавиатуру
        await query.message.reply_text(
            "Главное меню:",
            reply_markup=get_main_keyboard()
        )

    elif data == "back_planets":
        # КНОПКА "НАЗАД К ПЛАНЕТАМ" - ВОЗВРАЩАЕМ К ВЫБОРУ ПЛАНЕТ
        await query.edit_message_text(
            "🌌 *Выберите планету:*\n(8 планет Солнечной системы)",
            parse_mode='Markdown',
            reply_markup=get_planets_keyboard()
        )

    elif data == "back_compare":
        # Возврат к выбору сравнения
        await query.edit_message_text(
            "⚖️ *Выберите пару для сравнения:*",
            parse_mode='Markdown',
            reply_markup=get_compare_keyboard()
        )

    elif data == "back_tasks":
        # Возврат к выбору задач
        await query.edit_message_text(
            "📚 *Выберите тип задачи:*\n\n"
            "📐 *Космические скорости* - первая и вторая космические\n"
            "⚖️ *Сравнение масс* - отношения масс объектов\n"
            "🌍 *Сила тяжести* - ускорение свободного падения\n"
            "🔄 *Периоды* - орбитальные и синодические периоды\n"
            "⭐️ *Звездные задачи* - светимость, масса звезд\n"
            "📏 *Плотность* - средняя плотность планет",
            parse_mode='Markdown',
            reply_markup=get_tasks_keyboard()
        )


async def show_celestial_body_inline(query, body_name: str):
    """Показать информацию через инлайн-кнопку"""
    if body_name not in CELESTIAL_BODIES:
        await query.edit_message_text(
            f"❌ Объект '{body_name}' не найден.",
            parse_mode='Markdown'
        )
        return

    body = CELESTIAL_BODIES[body_name]

    response = f"{body['emoji']} *{body_name.upper()}* ({body['name_en']})\n\n"
    response += f"📌 *Тип:* {body['type']}\n\n"
    response += f"⚖️ *Масса:* {body['mass']}\n"
    response += f"📏 *Радиус:* {body['radius']}\n"

    if 'distance' in body:
        response += f"📍 *Расстояние:* {body['distance']}\n"

    if 'period' in body:
        response += f"🔄 *Период обращения:* {body['period']}\n"

    if 'luminosity' in body:
        response += f"☀️ *Светимость:* {body['luminosity']}\n"

    if 'temperature' in body:
        response += f"🌡️ *Температура:* {body['temperature']}\n"

    response += f"\n📊 *Точность:* {body['accuracy']}\n"
    response += f"📚 *Источники:* {body['sources']}\n\n"
    response += f"🎯 *{body['task']}*\n\n"
    response += body['solution']

    # Кнопка "Назад к планетам" - только для планет, не для звезд
    if body_name in ["Меркурий", "Венера", "Земля", "Марс", "Юпитер", "Сатурн", "Уран", "Нептун"]:
        keyboard = [[InlineKeyboardButton("🔙 Назад к планетам", callback_data="back_planets")]]
    else:
        keyboard = [[InlineKeyboardButton("🔙 Назад в меню", callback_data="back_main")]]

    reply_markup = InlineKeyboardMarkup(keyboard)

    await query.edit_message_text(response, parse_mode='Markdown', reply_markup=reply_markup)


async def show_comparison(query, body1: str, body2: str):
    """Показать сравнение"""
    if body1 not in CELESTIAL_BODIES or body2 not in CELESTIAL_BODIES:
        await query.edit_message_text("❌ Один из объектов не найден.")
        return

    b1 = CELESTIAL_BODIES[body1]
    b2 = CELESTIAL_BODIES[body2]

    response = f"📊 *СРАВНЕНИЕ: {b1['emoji']} {body1} vs {b2['emoji']} {body2}*\n\n"

    # Сравнение массы
    response += f"⚖️ *Масса:*\n• {body1}: {b1['mass']}\n• {body2}: {b2['mass']}\n\n"

    # Сравнение радиуса
    response += f"📏 *Радиус:*\n• {body1}: {b1['radius']}\n• {body2}: {b2['radius']}\n\n"

    # Особенности сравнения
    if body1 == "Земля" and body2 == "Марс":
        response += """📝 **Сравнение силы тяжести:**
g_Земля = 9.81 м/с²
g_Марс = 3.71 м/с²
Отношение: g_Марс/g_Земля = 3.71/9.81 ≈ 0.38

📐 **Формула сравнения:** g₁/g₂ = (M₁/M₂) × (R₂²/R₁²)

🎯 **Вывод:** Сила тяжести на Марсе составляет ~38% от земной
"""
    elif body1 == "Солнце" and body2 == "Сириус":
        response += """📝 **Сравнение светимости:**
L_Солнце = 1 L☉
L_Сириус = 25.4 L☉
Отношение: L_Сириус/L_Солнце = 25.4

📐 **Формула:** L ∝ M³·⁵ (для главной последовательности)

🎯 **Вывод:** Сириус в 25.4 раза ярче Солнца
"""
    elif body1 == "Юпитер" and body2 == "Сатурн":
        response += """📝 **Сравнение плотности:**
ρ_Юпитер = 1.33 г/см³
ρ_Сатурн = 0.69 г/см³
Отношение: ρ_Юпитер/ρ_Сатурн ≈ 1.93

📐 **Формула:** ρ = 3M/(4πR³)

🎯 **Вывод:** Юпитер почти в 2 раза плотнее Сатурна
"""

    keyboard = [[InlineKeyboardButton("🔙 Назад к сравнению", callback_data="back_compare")]]
    reply_markup = InlineKeyboardMarkup(keyboard)

    await query.edit_message_text(response, parse_mode='Markdown', reply_markup=reply_markup)


async def show_task_with_solution(query, task_type: str):
    """Показать задачу с полным решением"""
    tasks = {
        "velocity": """
🚀 **ЗАДАЧА: Космические скорости Марса**

📝 **Условие:**
Вычислите первую космическую скорость для Марса.

📐 **Формулы:**
1. Первая космическая: v₁ = √(GM/R)
2. Вторая космическая: v₂ = √(2GM/R)

🔢 **Данные:**
- G = 6.67430×10⁻¹¹ м³/(кг·с²)
- M_Марс = 6.4171×10²³ кг
- R_Марс = 3.3895×10⁶ м

📝 **Решение:**
1. Первая космическая:
   v₁ = √(6.67430×10⁻¹¹ × 6.4171×10²³ / 3.3895×10⁶)
   v₁ = √(1.264×10⁷) ≈ 3.56×10³ м/с

2. Вторая космическая:
   v₂ = √(2 × 6.67430×10⁻¹¹ × 6.4171×10²³ / 3.3895×10⁶)
   v₂ = √(2.528×10⁷) ≈ 5.03×10³ м/с

🎯 **Ответы:**
- Первая космическая: ~3.56 км/с
- Вторая космическая: ~5.03 км/с

⚡ **Интересный факт:** На Марсе запускать ракеты проще, чем на Земле!
""",
        "mass": """
⚖️ **ЗАДАЧА: Сравнение масс планет**

📝 **Условие:**
Во сколько раз масса Юпитера больше массы Сатурна?

📐 **Формула:**
N = M₁/M₂

🔢 **Данные:**
- M_Юпитер = 1.8982×10²⁷ кг
- M_Сатурн = 5.6834×10²⁶ кг

📝 **Решение:**
N = M_Юпитер / M_Сатурн
N = 1.8982×10²⁷ / 5.6834×10²⁶
N = 3.339

🎯 **Ответ:** Юпитер в 3.34 раза массивнее Сатурна

📝 **Дополнительная задача:**
Во сколько раз Солнце массивнее Земли?

🔢 **Решение:**
M_Солнце = 1.9885×10³⁰ кг
M_Земля = 5.9722×10²⁴ кг
N = 1.9885×10³⁰ / 5.9722×10²⁴ = 3.33×10⁵

🎯 **Ответ:** Солнце в 333 000 раз массивнее Земли
""",
        "gravity": """
🌍 **ЗАДАЧА: Сила тяжести на Венере**

📝 **Условие:**
Рассчитайте ускорение свободного падения на Венере.

📐 **Формула:**
g = GM/R²

🔢 **Данные:**
- G = 6.67430×10⁻¹¹ м³/(кг·с²)
- M_Венера = 4.8675×10²⁴ кг
- R_Венера = 6.0518×10⁶ м

📝 **Решение:**
g = (6.67430×10⁻¹¹ × 4.8675×10²⁴) / (6.0518×10⁶)²
g = 3.248×10¹⁴ / 3.663×10¹³
g ≈ 8.87 м/с²

🎯 **Ответ:** g_Венера ≈ 8.87 м/с²

📝 **Сравнение с Землей:**
g_Земля = 9.81 м/с²
Отношение: 8.87/9.81 ≈ 0.904

🎯 **Вывод:** На Венере сила тяжести ~90% от земной

⚡ **Интересный факт:** Несмотря на близкое g, атмосферное давление на Венере в 92 раза больше земного!
""",
        "period": """
🔄 **ЗАДАЧА: Орбитальные периоды**

📝 **Условие 1:**
Определите синодический период Венеры относительно Земли.

📐 **Формула:**
1/S = 1/T₁ - 1/T₂
где S - синодический период, T₁ и T₂ - сидерические периоды

🔢 **Данные:**
- T_Земля = 365.25 дней
- T_Венера = 224.7 дней

📝 **Решение:**
1/S = 1/224.7 - 1/365.25
1/S = 0.004451 - 0.002738 = 0.001713
S = 1/0.001713 ≈ 583.8 дней

🎯 **Ответ:** Синодический период Венеры ~584 дня

📝 **Условие 2:**
Проверьте третий закон Кеплера для Меркурия.

📐 **Формула:**
T²/a³ = const = 1 для Земли (в годах и а.е.)

🔢 **Данные:**
- T_Меркурий = 0.241 лет (88/365.25)
- a_Меркурий = 0.3871 а.е.

📝 **Решение:**
T²/a³ = (0.241)² / (0.3871)³
T²/a³ = 0.0581 / 0.0580 ≈ 1.002

🎯 **Ответ:** Закон выполняется с высокой точностью
""",
        "stars": """
⭐️ **ЗАДАЧА: Звездные характеристики**

📝 **Условие 1:**
Во сколько раз Сириус ярче Солнца?

🔢 **Данные:**
- L_Сириус = 25.4 L☉
- L_Солнце = 1 L☉

📝 **Решение:**
N = L_Сириус / L_Солнце = 25.4

🎯 **Ответ:** Сириус в 25.4 раза ярче Солнца

📝 **Условие 2:**
Оцените светимость звезды с массой 5 M☉.

📐 **Формула:** L ∝ M³·⁵ (зависимость масса-светимость для главной последовательности)

📝 **Решение:**
L/L☉ = (M/M☉)³·⁵ = 5³·⁵ = 5³ × √5 = 125 × 2.236 ≈ 279

🎯 **Ответ:** Звезда массой 5 M☉ имеет светимость ~279 L☉

📝 **Условие 3:**
Определите радиус Сириуса A в метрах.

🔢 **Данные:**
- R_Сириус A = 1.71 R☉
- R☉ = 6.957×10⁸ м

📝 **Решение:**
R = 1.71 × 6.957×10⁸ = 1.189×10⁹ м

🎯 **Ответ:** R_Сириус A ≈ 1.19×10⁹ м
""",
        "density": """
📏 **ЗАДАЧА: Плотность небесных тел**

📝 **Условие 1:**
Рассчитайте среднюю плотность Земли.

📐 **Формула:**
ρ = M/V = 3M/(4πR³)

🔢 **Данные:**
- M_Земля = 5.9722×10²⁴ кг
- R_Земля = 6.371×10⁶ м

📝 **Решение:**
V = (4/3)πR³ = (4/3)π × (6.371×10⁶)³ ≈ 1.083×10²¹ м³
ρ = 5.9722×10²⁴ / 1.083×10²¹ ≈ 5515 кг/м³

🎯 **Ответ:** ρ_Земля ≈ 5.52 г/см³

📝 **Условие 2:**
Сравните плотность Сатурна с плотностью воды.

🔢 **Данные:**
- ρ_Сатурн = 687 кг/м³
- ρ_вода = 1000 кг/м³

📝 **Решение:**
Отношение: 687/1000 = 0.687

🎯 **Ответ:** Сатурн менее плотный, чем вода (~69% плотности воды)

📝 **Условие 3:**
Оцените плотность нейтронной звезды (радиус 10 км, масса 1.4 M☉).

🔢 **Данные:**
- M = 1.4 × 1.989×10³⁰ = 2.785×10³⁰ кг
- R = 1.0×10⁴ м

📝 **Решение:**
ρ = 3M/(4πR³) = 3×2.785×10³⁰/(4π×(1.0×10⁴)³)
ρ ≈ 6.65×10¹⁷ кг/м³

🎯 **Ответ:** ρ ≈ 6.7×10¹⁷ кг/м³ (в миллионы раз плотнее обычного вещества!)
"""
    }

    if task_type in tasks:
        response = tasks[task_type]
        response += "\n\n🔍 *Используйте данные из бота для решения своих задач!*"
    else:
        response = "📝 Выберите тип задачи из списка выше"

    keyboard = [
        [InlineKeyboardButton("🔄 Новая задача", callback_data=f"task_{task_type}"),
         InlineKeyboardButton("🔙 Назад к задачам", callback_data="back_tasks")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)

    await query.edit_message_text(response, parse_mode='Markdown', reply_markup=reply_markup)


async def show_methods(update: Update):
    """Показать методы измерений"""
    methods = """
🔬 *МЕТОДЫ АСТРОНОМИЧЕСКИХ ИЗМЕРЕНИЙ*

*📡 Определение массы:*
• Планеты: по движению спутников (формула: M = 4π²a³/(GT²))
• Звезды в двойных системах: третий закон Кеплера
• Одиночные звезды: эволюционные модели + спектральный класс

*📏 Определение радиуса:*
• Радиолокация (планеты): τ = 2R/c
• Интерферометрия (звезды): θ = 1.22λ/D
• Затменные двойные системы: по кривой блеска
• Угловой диаметр + параллакс: R = θ·d/2

*☀️ Определение светимости:*
• Фотометрия + параллакс: L = 4πd²F
• Болометрические измерения: интеграл по всему спектру
• Модели атмосфер звезд: закон Стефана-Больцмана L = 4πR²σT⁴

*📍 Определение расстояния:*
• Тригонометрический параллакс: d = 1/p (пк)
• Спектроскопический параллакс: по спектру и светимости
• Цефеиды: период-светимость P-L relation
• Красное смещение: закон Хаббла v = H₀d

*🎯 Точность в олимпиадах:*
1. Всегда указывайте погрешность!
2. Используйте систему СИ
3. Сравнивайте разные источники
4. Учитывайте метод измерения
"""
    await update.message.reply_text(methods, parse_mode='Markdown', reply_markup=get_main_keyboard())


async def show_help(update: Update):
    """Показать помощь"""
    help_text = """
❓ *ПОМОЩЬ ПО ИСПОЛЬЗОВАНИЮ ASTROBOT*

*Основные функции:*
• 🪐 **8 Планет** - полная информация о планетах Солнечной системы
• ⭐️ **Сириус** - подробные данные о самой яркой звезде
• ☀️ **Солнце** - параметры нашей звезды
• 📊 **Сравнить** - сравнение двух небесных тел
• 📝 **Задачи** - олимпиадные задачи с полными решениями
• 🔬 **Методы** - методики астрономических измерений

*🎯 Для олимпиад:*
• Все задачи содержат полное решение
• Указаны все используемые формулы
• Даны численные значения констант
• Приведены промежуточные расчеты

*📚 Константы для расчетов:*
• G = 6.67430×10⁻¹¹ м³/(кг·с²) (гравитационная постоянная)
• σ = 5.670374×10⁻⁸ Вт/(м²·К⁴) (постоянная Стефана-Больцмана)
• 1 а.е. = 149 597 870 700 м (астрономическая единица)
• 1 пк = 3.085677581×10¹⁶ м = 3.261563776 св. лет (парсек)
• M☉ = 1.9885×10³⁰ кг (масса Солнца)
• R☉ = 6.957×10⁸ м (радиус Солнца)
• L☉ = 3.828×10²⁶ Вт (светимость Солнца)

*✅ Особенности:*
• К каждой задаче прилагается решение
• Показаны все шаги расчета
• Даны интересные факты
• Формулы указаны в решениях задач
"""
    await update.message.reply_text(help_text, parse_mode='Markdown', reply_markup=get_main_keyboard())


# ==================== ЗАПУСК БОТА ====================

def main():
    """Запуск бота"""
    print("=" * 60)
    print("🚀 AstroBot: Полный справочник по астрономии с решениями")
    print("=" * 60)
    print("✅ К ВСЕМ задачам добавлены решения и формулы")
    print("✅ Удалена дублирующая строка 'Формула:' в конце")
    print("✅ Формулы остались только в решениях задач")
    print("✅ Кнопка 'Назад к планетам' работает")
    print("=" * 60)
    print("⚠️  ВАЖНО: Замените TOKEN на ваш реальный токен!")
    print("=" * 60)

    try:
        # Создаем приложение
        application = Application.builder().token(TOKEN).build()
        await application.updater.start_polling(allowed_updates=Update.ALL_TYPES, drop_pending_updates=True)

        # Регистрируем обработчики
        application.add_handler(CommandHandler("start", start_command))
        application.add_handler(CallbackQueryHandler(button_callback))
        application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))

        print("Бот запускается...")
        print("1. Замените TOKEN в строке 24")
        print("2. Запустите код в PyCharm (зеленая кнопка ▶️)")
        print("3. Найдите бота в Telegram")
        print("4. Напишите /start")
        print("5. Проверьте кнопку '8 Планет' - нет дублирования формулы!")
        print("=" * 60)
# ... весь ваш код ...

def main():
    # ... функция main ...

# ==== ЭТА СТРОКА ДОЛЖНА БЫТЬ БЕЗ ОТСТУПОВ ====
if __name__ == '__main__':
    # Эта строка с 4 пробелами
    import time
    # Эта строка с 4 пробелами
    while True:
        # Эта строка с 8 пробелами
        try:
            # Эта строка с 12 пробелами
            main()
        # Эта строка с 8 пробелами
        except KeyboardInterrupt:
            # Эта строка с 12 пробелами
            print("\n🛑 Остановка бота...")
            # Эта строка с 12 пробелами
            break
        # Эта строка с 8 пробелами
        except Exception as e:
            # Эта строка с 12 пробелами
            print(f"❌ Ошибка: {e}")
            # Эта строка с 12 пробелами
            print("🔄 Перезапуск через 5 секунд...")
            # Эта строка с 12 пробелами
            time.sleep(5)







